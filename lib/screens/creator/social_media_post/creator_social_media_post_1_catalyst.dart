...
class _CreatorSocialMediaPostCatalystState
  ...
  SocialMediaPlatforms? _postPlatform;
  final FocusNode _platformNode = FocusNode(); // hack for non-text field inputs
  bool _isPlatformAutoGenerated = false;
  ...
  void _handleChangeCatalyst() async {
    ...
    setState(() {
      ...
      if (_isPlatformAutoGenerated) {
        _postPlatform = widget.catalyst.derivedPlatforms.isNotEmpty
            ? widget.catalyst.derivedPlatforms[0]
            : null;
      }
      if (widget.catalyst.derivedPlatforms.isNotEmpty) {
        _isPlatformAutoGenerated = true;
      }
    });
    _catalystNode.requestFocus(); // hack to stop losing focus on setState()
  }

  void _handleChangeSelectedPlatform(dynamic newPlatformString) {
    SocialMediaPlatforms smp = socialMediaPlatformsOptions.keys.firstWhere(
      (k) => socialMediaPlatformsOptions[k] == newPlatformString,
      orElse: () => SocialMediaPlatforms.instagram,
    );

    widget.updateCatalyst(platforms: [smp]);
    setState(() {
      _postPlatform = smp;
      _isPlatformAutoGenerated = false;
    });
  }
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            "What account do you want to post on?",
            style: AppText.bodyBold,
          ),
          Padding(
            padding: const EdgeInsets.only(top: 10, bottom: 30),
            child: Column(
              ...
              children: [
                ...
                if (_isPlatformAutoGenerated) const SmartAutofillText(),
              ],
            ),
          ),
          const Text(
          ...
          SizedBox(
            ...
            child: CustomButton(
              handlePressed: () {
                ...
                // Validate returns true if the form is valid, or false otherwise.
                bool platformsErrorCheck = _postPlatform == null;
                bool datesErrorCheck = _dateController.text == "";

                if (platformsErrorCheck) {
                  _platformNode.requestFocus();
